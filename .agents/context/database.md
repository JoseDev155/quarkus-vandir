# Contexto de la Base de Datos: Vandir Store

El sistema utiliza MySQL. A continuación se presenta el script DDL oficial. 

### Reglas de Negocio Clave a considerar:
* **Roles:** Existen 3 roles inmutables: `Administrador` (Acceso total, configuración, respaldos), `Gerente` (Reportes, inventario, órdenes de compra) y `Vendedor` (Punto de venta, clientes).
* **Alertas de Stock:** Si `stock_actual` <= `stock_minimo` en la tabla `productos`, el sistema debe tratarlo como una alerta crítica en la UI.
* **Precios Históricos:** La tabla `detalle_ventas` guarda el `precio_aplicado` para evitar que cambios futuros en el precio del producto alteren el historial de ventas.
